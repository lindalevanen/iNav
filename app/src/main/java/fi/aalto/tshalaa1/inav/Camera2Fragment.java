...
public class Camera2Fragment extends Fragment
        implements Navigator.NavigatorListener {
    ...
    private final File NAVIGATION_CLIENT_MAIN_FOLDER = new File(Environment.getExternalStorageDirectory(), "iNav Client TEST");
    ...
    private JSONDataCollector json;
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
             ...
             */
            json = new JSONDataCollector(false, false, false, true, false) {
                @Override
...
//                    Navigation Client
            ...
            mFile =new File(NAVIGATION_CLIENT_MAIN_FOLDER, imgId + ".jpg");
...
//            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile));
    ...
    public void onLocated(Point3D location) {
        ...
        if(location == null && mTextureView != null) {
...
//                    mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER, CaptureRequest.CONTROL_AF_TRIGGER_START);
                    ...
                    Integer previewafState = result.get(CaptureResult.CONTROL_AF_STATE);
                    Log.d(TAG, "previewafState: " +  result.get(CaptureResult.CONTROL_AF_STATE));
                    if((previewafState == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED || previewafState == CaptureResult.CONTROL_AF_STATE_PASSIVE_FOCUSED) && sda.preview){
                        Log.d(TAG, "picTaken afState focused with afState: " + previewafState);
                        captureStillPicture();
                        sda.preview = false;
                        sda.picTaken = true;
                    }
    ...
    private int displayWidth = Utils.getContext().getResources().getDisplayMetrics().widthPixels;
    private int displayHeight = Utils.getContext().getResources().getDisplayMetrics().heightPixels;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        camView = inflater.inflate(R.layout.camera_layout, container, false);
        camView.getLayoutParams().height = displayHeight/2;
        camView.getLayoutParams().width = displayWidth;
...
//                mState = STATE_PREVIEW;
        ...
        updateTextureViewSize(displayWidth, displayHeight);
...
//    https://www.binpress.com/tutorial/video-cropping-with-texture-view/21
    private void updateTextureViewSize(int viewWidth, int viewHeight) {
        ...
        if (camView.getLayoutParams().width  > viewWidth && camView.getLayoutParams().height > viewHeight) {
            scaleX = camView.getLayoutParams().width  / (float) viewWidth;
            scaleY = camView.getLayoutParams().height / (float) viewHeight;
        } else if (camView.getLayoutParams().width  < viewWidth && camView.getLayoutParams().height < viewHeight) {
            scaleY = (float) viewWidth / (float) camView.getLayoutParams().width ;
            scaleX = (float) viewHeight / (float) camView.getLayoutParams().height;
        } else if (viewWidth > camView.getLayoutParams().width ) {
            System.out.println("zero? " + camView.getLayoutParams().width + ", " + camView.getLayoutParams().height );
            System.out.println("zero? " +viewWidth+ ", " +viewHeight );
            scaleY = (viewWidth / camView.getLayoutParams().width ) / (((float) viewHeight) / (float) camView.getLayoutParams().height);
        } else if (viewHeight > camView.getLayoutParams().height) {
            scaleX = ((float) viewHeight / camView.getLayoutParams().width ) / ((float) viewWidth / camView.getWidth());
        }
...
//        mFile = new File(getActivity().getExternalFilesDir(null), "pic.jpg");
    ...
    public void onResume() {
        ...
        if (mTextureView.isAvailable()) {
            openCamera(camView.getWidth(), camView.getHeight());
        } else {
    ...
    public void onDestroy() {
        ...
        if(json != null) json.unregisterReceiverWifi();
        super.onDestroy();
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                //TODO: set your preferred image size here
                //Refer to https://developer.android.com/reference/android/hardware/camera2/CameraCharacteristics.html#SCALER_STREAM_CONFIGURATION_MAP
                Size preferredJPEGsize = new Size(1280, 720);
                List<Size> jpegSizes = Arrays.asList(map.getOutputSizes(ImageFormat.JPEG));
                Collections.reverse( jpegSizes);
                for(Size s : jpegSizes){
                    if(s.getWidth() <= preferredJPEGsize.getWidth() && s.getHeight() <= preferredJPEGsize.getHeight()) {
                        optimal = s;
                    }
                }
    ...
    private void openCamera(int width, int height) {
        ...
        if(sda != null) sda.picTaken = true;


        Log.d("Camera2Fragment", "picTaken in openCamera: " + sda.picTaken);
...
//            buffer.get(bytes);
            Log.d(TAG, "mFile" + mImageBytes.length );
...
//                mImage.close();
                if (null != output) {
                    try {
                        output.close();
                        MediaScannerConnection.scanFile(Utils.getContext(), new String[] { mFile.getPath() }, null, new MediaScannerConnection.OnScanCompletedListener() {
                            /*
                             *   (non-Javadoc)
